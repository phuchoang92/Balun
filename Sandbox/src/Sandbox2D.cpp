...
void Sandbox2D::OnUpdate(Balun::Timestep ts)
{
...

	{
		...
		Balun::Renderer2D::DrawRotatedQuad({ 1.0f, 0.0f }, { 0.8f, 0.8f }, -45.0f, { 0.8f, 0.2f, 0.3f, 1.0f });
		...
		Balun::Renderer2D::DrawQuad({ 0.0f, 0.0f, -0.1f }, { 20.0f, 20.0f }, m_CheckboardTexture, 10.0f);
		Balun::Renderer2D::DrawRotatedQuad({ -2.0f, 0.0f, 0.0f }, { 1.0f, 1.0f }, rotation, m_CheckboardTexture, 20.0f);		
		...
		{
			for (float x = -0.5; x < 5.0f; x += 0.1f)
			{
				glm::vec4 color = { (x + 5.0f) / 10.0f, 0.4f, (y + 5.0f) / 10.0f, 0.7f };
				Balun::Renderer2D::DrawQuad({ x,y }, {0.45f, 0.45f},color);
			}
		}
	...
	if (dockingEnabled)
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				if (ImGui::MenuItem("Exit")) Balun::Application::Get().Close();
				ImGui::EndMenu();
			}
		...
		auto stats = Balun::Renderer2D::GetStats();
