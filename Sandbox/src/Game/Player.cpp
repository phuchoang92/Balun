...
{
	m_ShipTexture = Texture2D::Create("assets/textures/Player.png");
}

void Player::OnUpdate(Balun::Timestep ts)
{
	m_Time += ts;

	if (Input::IsKeyPressed(BL_KEY_SPACE))
	{
	...
	else if (Input::IsKeyPressed(BL_KEY_RIGHT))
	{
		m_Position.x += 0.1f;
	}
	else if (Input::IsKeyPressed(BL_KEY_LEFT))
	{
		m_Position.x -= 0.1f;
	}
	else if (Input::IsKeyPressed(BL_KEY_UP))
	{
		m_Position.y += 0.1f;
	}
	else if (Input::IsKeyPressed(BL_KEY_DOWN))
	{
		m_Position.y -= 0.1f;
	}
	//else
...
void Player::OnRender()
{
	...
	//Renderer2D::DrawQuad({ m_Position.x, m_Position.y, 0.5f }, { 4.0f, 4.0f }, glm::radians(GetRotation()), m_ShipTexture);
}
